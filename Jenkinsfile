                ...
                stage('Android Release') {
                    ...
                    steps {
                        ...
                        withCredentials([string(credentialsId: 'ANDROID_STOREPASS', variable: 'ANDROID_STOREPASS')]) {
                            sh 'mkdir build; cd build; ${QT_PATH}/${QMAKE_VER} -r ${WORKSPACE}/qgroundcontrol.pro LIBS+=" -L/curl-android-ios-cURL_7.60.0/prebuilt-with-ssl/android/armeabi-v7a/" INCLUDEPATH+=" /curl-android-ios-cURL_7.60.0/prebuilt-with-ssl/android/include/ " CONFIG+=installer CONFIG+=${QGC_CONFIG}'
