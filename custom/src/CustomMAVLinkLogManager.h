...
#ifndef CustomMAVLinkLogManager_H
#define CustomMAVLinkLogManager_H
...
Q_DECLARE_LOGGING_CATEGORY(CustomMAVLinkLogManagerLog)

class QNetworkAccessManager;
class CustomMAVLinkLogManager;

//-----------------------------------------------------------------------------
class CustomMAVLinkLogFiles : public QObject
{
    Q_OBJECT
public:
    CustomMAVLinkLogFiles    (CustomMAVLinkLogManager* manager, const QString& filePath, bool newFile = false);
...
private:
    CustomMAVLinkLogManager*  _manager;
    QString             _name;
    quint32             _size;
    bool                _selected;
    bool                _uploading;
...
//-----------------------------------------------------------------------------
class CustomMAVLinkLogProcessor
{
public:
    CustomMAVLinkLogProcessor();
    ~CustomMAVLinkLogProcessor();
    void                close       ();
    bool                valid       ();
    bool                create      (CustomMAVLinkLogManager *manager, const QString path, uint8_t id);
    CustomMAVLinkLogFiles*    record      () { return _record; }
...
private:
    ...
    QString             _fileName;
    CustomMAVLinkLogFiles*    _record;
};

//-----------------------------------------------------------------------------
class CustomMAVLinkLogManager : public QGCTool
{
    Q_OBJECT

public:
    CustomMAVLinkLogManager    (QGCApplication* app, QGCToolbox* toolbox);
...
private:
    ...
    void _insertNewLog              (CustomMAVLinkLogFiles* newLog);
    void _deleteLog                 (CustomMAVLinkLogFiles* log);
    void _discardLog                ();
...
private:
    ...
    CustomMAVLinkLogFiles*        _currentLogfile;
    Vehicle*                _vehicle;
    bool                    _logRunning;
    bool                    _loggingDisabled;
    CustomMAVLinkLogProcessor*    _logProcessor;
