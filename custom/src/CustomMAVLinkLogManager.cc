...
#include "CustomMAVLinkLogManager.h"
...
QGC_LOGGING_CATEGORY(CustomMAVLinkLogManagerLog, "CustomMAVLinkLogManagerLog")
...
//-----------------------------------------------------------------------------
CustomMAVLinkLogFiles::CustomMAVLinkLogFiles(CustomMAVLinkLogManager* manager, const QString& filePath, bool newFile)
    : _manager(manager)
    , _size(0)
    , _selected(false)
    , _uploading(false)
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogFiles::setSize(quint32 size)
{
    _size = size;
    emit sizeChanged();
}
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogFiles::setSelected(bool selected)
{
    _selected = selected;
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogFiles::setUploading(bool uploading)
{
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogFiles::setProgress(qreal progress)
{
    _progress = progress;
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogFiles::setWriting(bool writing)
{
    _writing = writing;
    emit writingChanged();
}
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogFiles::setUploaded(bool uploaded)
{
    _uploaded = uploaded;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CustomMAVLinkLogProcessor::CustomMAVLinkLogProcessor()
    : _fd(nullptr)
    , _written(0)
    , _sequence(-1)
    , _numDrops(0)
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogProcessor::close()
{
    if(_fd) {
        fclose(_fd);
        _fd = nullptr;
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogProcessor::valid()
{
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogProcessor::create(CustomMAVLinkLogManager* manager, const QString path, uint8_t id)
{
    ...
    if(_fd) {
        _record = new CustomMAVLinkLogFiles(manager, _fileName, true);
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogProcessor::_checkSequence(uint16_t seq, int& num_drops)
{
    num_drops = 0;
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogProcessor::_writeData(void* data, int len)
{
    if(!_error) {
        ...
        if(!_error) {
            ...
            qCDebug(CustomMAVLinkLogManagerLog) << "File IO error:" << len << "bytes into" << _fileName;
        }
    }
}
...
//-----------------------------------------------------------------------------
QByteArray
CustomMAVLinkLogProcessor::_writeUlogMessage(QByteArray& data)
{
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogProcessor::processStreamData(uint16_t sequence, uint8_t first_message, QByteArray data)
{
    int num_drops = 0;
    _error = false;
    while(_checkSequence(sequence, num_drops)) {
        ...
        if(!_gotHeader) {
            if(data.size() < 16) {
                ...
                qCWarning(CustomMAVLinkLogManagerLog) << "Corrupt log header. Canceling log download.";
                return false;
            }
            //-- Write header
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CustomMAVLinkLogManager::CustomMAVLinkLogManager(QGCApplication* app, QGCToolbox* toolbox)
    : QGCTool(app, toolbox)
...
//-----------------------------------------------------------------------------
CustomMAVLinkLogManager::~CustomMAVLinkLogManager()
{
    _logFiles.clear();
}
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setToolbox(QGCToolbox* toolbox)
{
    QGCTool::setToolbox(toolbox);
    QQmlEngine::setObjectOwnership(this, QQmlEngine::CppOwnership);
    qmlRegisterUncreatableType<CustomMAVLinkLogManager>("QGroundControl.CustomMAVLinkLogManager", 1, 0, "CustomMAVLinkLogManager", "Reference only");
    //-- Logging location
    _ulogExtension  = ".";
    ...
    if(!QDir(_logPath).exists()) {
        if(!QDir().mkpath(_logPath)) {
            qCWarning(CustomMAVLinkLogManagerLog) << "Could not create MAVLink log download path:" << _logPath;
    ...
    if(!_loggingDisabled) {
        ...
        while(it.hasNext()) {
            _insertNewLog(new CustomMAVLinkLogFiles(this, it.next()));
        }
        qCDebug(CustomMAVLinkLogManagerLog) << "MAVLink logs directory:" << _logPath;
        connect(toolbox->multiVehicleManager(), &MultiVehicleManager::activeVehicleChanged, this, &CustomMAVLinkLogManager::_activeVehicleChanged);
    }
}

//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setEmailAddress(QString email)
{
    _emailAddress = email;
    QSettings settings;
    settings.beginGroup(kMAVLinkLogGroup);
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setDescription(QString description)
{
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setUploadURL(QString url)
{
    _uploadURL = url;
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setFeedback(QString fb)
{
    _feedback = fb;
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setVideoURL(QString url)
{
    _videoURL = url;
    QSettings settings;
    settings.beginGroup(kMAVLinkLogGroup);
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setEnableAutoUpload(bool enable)
{
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setEnableAutoStart(bool enable)
{
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setDeleteAfterUpload(bool enable)
{
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setWindSpeed(int speed)
{
    _windSpeed = speed;
    QSettings settings;
    settings.beginGroup(kMAVLinkLogGroup);
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setRating(QString rate)
{
    _rating = rate;
    QSettings settings;
    settings.beginGroup(kMAVLinkLogGroup);
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::setPublicLog(bool pub)
{
    _publicLog = pub;
    QSettings settings;
    settings.beginGroup(kMAVLinkLogGroup);
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogManager::uploading()
{
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::uploadLog()
{
    if(_currentLogfile) {
        _currentLogfile->setUploading(false);
    }
    for(int i = 0; i < _logFiles.count(); i++) {
        _currentLogfile = qobject_cast<CustomMAVLinkLogFiles*>(_logFiles.get(i));
        if (_currentLogfile) {
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_insertNewLog(CustomMAVLinkLogFiles* newLog)
{
    ...
    if(!count) {
        ...
        for(int i = 0; i < count; i++) {
            CustomMAVLinkLogFiles* f = qobject_cast<CustomMAVLinkLogFiles*>(_logFiles.get(i));
...
//-----------------------------------------------------------------------------
int
CustomMAVLinkLogManager::_getFirstSelected()
{
    for(int i = 0; i < _logFiles.count(); i++) {
        CustomMAVLinkLogFiles* f = qobject_cast<CustomMAVLinkLogFiles*>(_logFiles.get(i));
        if (f) {
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::deleteLog()
{
    while (true) {
        ...
        CustomMAVLinkLogFiles* log = qobject_cast<CustomMAVLinkLogFiles*>(_logFiles.get(idx));
        _deleteLog(log);
    }
}
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_deleteLog(CustomMAVLinkLogFiles* log)
{
    QString filePath = _makeFilename(log->name());
    QFile gone(filePath);
    if(!gone.remove()) {
        qCWarning(CustomMAVLinkLogManagerLog) << "Could not delete MAVLink log file:" << _logPath;
    }
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::cancelUpload()
{
    for(int i = 0; i < _logFiles.count(); i++) {
        CustomMAVLinkLogFiles* pLogFile = qobject_cast<CustomMAVLinkLogFiles*>(_logFiles.get(i));
        if (pLogFile) {
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::startLogging()
{
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::stopLogging()
{
...
//-----------------------------------------------------------------------------
QHttpPart
custom_create_form_part(const QString& name, const QString& value)
{
    QHttpPart formPart;
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogManager::_sendLog(const QString& logFile)
{
    QString defaultDescription = _description;
    if(_description.isEmpty()) {
        qCWarning(CustomMAVLinkLogManagerLog) << "Log description missing. Using defaults.";
        defaultDescription = kDefaultDescr;
    }
    if(_emailAddress.isEmpty()) {
        qCWarning(CustomMAVLinkLogManagerLog) << "User email missing.";
        return false;
    }
    if(_uploadURL.isEmpty()) {
        qCWarning(CustomMAVLinkLogManagerLog) << "Upload URL missing.";
        return false;
    }
    QFileInfo fi(logFile);
    if(!fi.exists()) {
        qCWarning(CustomMAVLinkLogManagerLog) << "Log file missing:" << logFile;
        return false;
    }
    ...
    if(!file || !file->open(QIODevice::ReadOnly)) {
        ...
        qCWarning(CustomMAVLinkLogManagerLog) << "Could not open log file:" << logFile;
        return false;
    }
    if(!_nam) {
    ...
    QHttpPart emailPart = custom_create_form_part("email", _emailAddress);
    QHttpPart descriptionPart = custom_create_form_part("description", _description);
    QHttpPart sourcePart      = custom_create_form_part("source", "QGroundControl");
    QHttpPart versionPart     = custom_create_form_part("version", _app->applicationVersion());
    QHttpPart typePart        = custom_create_form_part("type", "flightreport");
    QHttpPart windPart        = custom_create_form_part("windSpeed", QString::number(_windSpeed));
    QHttpPart ratingPart      = custom_create_form_part("rating", _rating);
    QHttpPart publicPart      = custom_create_form_part("public", _publicLog ? "true" : "false");
    ...
    if(_feedback.isEmpty()) {
        feedbackPart = custom_create_form_part(kFeedback, "None Given");
    } else {
        feedbackPart = custom_create_form_part(kFeedback, _feedback);
    }
    multiPart->append(feedbackPart);
    QHttpPart videoPart;
    if(_videoURL.isEmpty()) {
        videoPart = custom_create_form_part(kVideoURL, "None");
    } else {
        videoPart = custom_create_form_part(kVideoURL, _videoURL);
    }
    ...
    connect(reply, &QNetworkReply::finished,  this, &CustomMAVLinkLogManager::_uploadFinished);
    connect(this, &CustomMAVLinkLogManager::abortUpload, reply, &QNetworkReply::abort);
    //connect(reply, &QNetworkReply::readyRead, this, &CustomMAVLinkLogManager::_dataAvailable);
    connect(reply, &QNetworkReply::uploadProgress, this, &CustomMAVLinkLogManager::_uploadProgress);
    multiPart->setParent(reply);
    qCDebug(CustomMAVLinkLogManagerLog) << "Log" << fi.baseName() << "Uploading." << fi.size() << "bytes.";
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogManager::_processUploadResponse(int http_code, QByteArray& data)
{
    qCDebug(CustomMAVLinkLogManagerLog) << "Uploaded response:" << QString::fromUtf8(data);
    emit readyRead(data);
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_dataAvailable()
{
    ...
    qCDebug(CustomMAVLinkLogManagerLog) << "Uploaded response data:" << QString::fromUtf8(data);
}

//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_uploadFinished()
{
    QNetworkReply* reply = qobject_cast<QNetworkReply*>(sender());
    if(!reply) {
        return;
    ...
    if(_processUploadResponse(http_code, data)) {
        qCDebug(CustomMAVLinkLogManagerLog) << "Log uploaded.";
        emit succeed();
        ...
        qCWarning(CustomMAVLinkLogManagerLog) << QString("Log Upload Error: %1 status: %2").arg(reply->errorString(), reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toString());
        emit failed();
    }
    reply->deleteLater();
    //-- Next (if any)
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_uploadProgress(qint64 bytesSent, qint64 bytesTotal)
{
    if(bytesTotal) {
    ...
    qCDebug(CustomMAVLinkLogManagerLog) << bytesSent << "of" << bytesTotal;
}

//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_activeVehicleChanged(Vehicle* vehicle)
{
    ...
    if(_vehicle && _vehicle->px4Firmware()) {
        disconnect(_vehicle, &Vehicle::armedChanged,        this, &CustomMAVLinkLogManager::_armedChanged);
        disconnect(_vehicle, &Vehicle::mavlinkLogData,      this, &CustomMAVLinkLogManager::_mavlinkLogData);
        disconnect(_vehicle, &Vehicle::mavCommandResult,    this, &CustomMAVLinkLogManager::_mavCommandResult);
        _vehicle = nullptr;
    ...
    if(vehicle && vehicle->px4Firmware()) {
        ...
        _logginDenied = false;
        connect(_vehicle, &Vehicle::armedChanged,       this, &CustomMAVLinkLogManager::_armedChanged);
        connect(_vehicle, &Vehicle::mavlinkLogData,     this, &CustomMAVLinkLogManager::_mavlinkLogData);
        connect(_vehicle, &Vehicle::mavCommandResult,   this, &CustomMAVLinkLogManager::_mavCommandResult);
        emit canStartLogChanged();
    }
}
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_mavlinkLogData(Vehicle* /*vehicle*/, uint8_t /*target_system*/, uint8_t /*target_component*/, uint16_t sequence, uint8_t first_message, QByteArray data, bool /*acked*/)
{
    if(_logProcessor && _logProcessor->valid()) {
        if(!_logProcessor->processStreamData(sequence, first_message, data)) {
            qCWarning(CustomMAVLinkLogManagerLog) << "Error writing MAVLink log file:" << _logProcessor->fileName();
            delete _logProcessor;
            _logProcessor = nullptr;
            _logRunning = false;
            _vehicle->stopMavlinkLog();
        ...
        qCWarning(CustomMAVLinkLogManagerLog) << "MAVLink log data received when not expected.";
    }
}

//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_mavCommandResult(int vehicleId, int component, int command, int result, bool noReponseFromVehicle)
{
    Q_UNUSED(vehicleId);
    Q_UNUSED(component);
    ...
    if(command == MAV_CMD_LOGGING_START || command == MAV_CMD_LOGGING_STOP) {
        ...
        if(result != MAV_RESULT_ACCEPTED) {
            if(command == MAV_CMD_LOGGING_STOP) {
                ...
                qCWarning(CustomMAVLinkLogManagerLog) << "Stop MAVLink log command failed.";
            } else {
                //-- Could not start logging for some reason.
                if(result == MAV_RESULT_DENIED) {
                    _logginDenied = true;
                    qCWarning(CustomMAVLinkLogManagerLog) << "Start MAVLink log command denied.";
                } else {
                    qCWarning(CustomMAVLinkLogManagerLog) << "Start MAVLink log command failed.";
                }
                _discardLog();
            }
        }
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_discardLog()
{
...
//-----------------------------------------------------------------------------
bool
CustomMAVLinkLogManager::_createNewLog()
{
    if(_logProcessor) {
        delete _logProcessor;
        _logProcessor = nullptr;
    }
    _logProcessor = new CustomMAVLinkLogProcessor;
    if(_logProcessor->create(this, _logPath, static_cast<uint8_t>(_vehicle->id()))) {
        _insertNewLog(_logProcessor->record());
        emit logFilesChanged();
    } else {
        qCWarning(CustomMAVLinkLogManagerLog) << "Could not create MAVLink log file:" << _logProcessor->fileName();
        delete _logProcessor;
        _logProcessor = nullptr;
    }
...
//-----------------------------------------------------------------------------
void
CustomMAVLinkLogManager::_armedChanged(bool armed)
{
    if(_vehicle && _vehicle->px4Firmware()) {
        if(armed) {
            if(_enableAutoStart) {
...
//-----------------------------------------------------------------------------
QString
CustomMAVLinkLogManager::_makeFilename(const QString& baseName)
{
